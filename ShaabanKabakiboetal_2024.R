...
# antigen-specific T cells by a multiplexed AIM assay. Cell Reports Met, 4, 100690 (2024).
...
### To modify ###
data_dir <- "/Unsupervised Analysis"
today <- "240129"
...
# Function 2 out of 2
flowSet_excl_1 <- function(flowSet_ICS){
  # (flowSet) -> flowSet
  #
  # Input:
  #   - flowSet_ICS: flowSet of the ICS data, containing all monkeys
  #
  # Returns a flowSet excluding monkeys who only have 1 gated event
  
  num_gated_events <- fsApply(flowSet_ICS, function(ff) { # Get total number of gated events
    nrow(ff)
  })
  
  pos <- which(num_gated_events[,1] == 1) # Get pos of monkeys who only have 1 gated event
  if (length(pos) > 0){
    new_flowSet <- flowSet_ICS[-pos]
  }else{
    new_flowSet <- flowSet_ICS
  }
  return(new_flowSet)
}
...
YN_cyt <- list()
cutoff_values <- as.data.frame(cbind(798, 1002, 517, 3500, 649)) # Determined with FlowJo
cutoff_values <- as.data.frame(rbind(cutoff_values, cbind(818, 843, 510, 3500, 750))) # Determined with FlowJo
...
for (i in 1:length(new_DataList)){
  ...
  for (j in 1:length(params)){
    ...
    df_cyt[Y_pos, j] <- "Y" # Write Y if pos. for cytokine
    df_cyt[N_pos, j] <- "N" # Write N if neg. for cytokine
  }
  YN_cyt[[i]] <- df_cyt
}
...
# Read all ICS files as a FlowSET
fs.ics <- read.flowSet(paste0(newdir, "/", fcs_files), ignore.text.offset = T, truncate_max_range = FALSE)
...
fs <- FlowSOM(fs.ics, compensate = FALSE, transform = TRUE, toTransform = ids, scale = TRUE, colsToUse = ids, nClus = 10)
...
# Clean names
names(fs$metaData) <- fs.ics@phenoData@data[["name"]] # Assign donor's name to metaData
...
umap_total <- ggplot(dat.num, aes(UMAP_1, UMAP_2)) +
  geom_point(aes(col = pheno_cluster), size = 0.5) +
  theme_bw() +
  ...
  geom_density_2d(alpha = 0.3, col = "grey40", h = 3) + # Contour plots
...
fig3a <- ggplot(fig3a_data, aes(UMAP_1, UMAP_2)) +
  geom_point(aes(col = pheno_cluster), size = 0.15) +
  ...
  scale_y_continuous(breaks = pretty_breaks(), limits = c(min(dat.num$UMAP_2), max(dat.num$UMAP_2))) + # To remove decimals
  scale_x_continuous(limits = c(min(dat.num$UMAP_1), max(dat.num$UMAP_1))) +
  geom_density_2d(alpha = 0.3, color = "grey40", h = 3) + # Contour plots
...
ggsave(file = paste0("Main figures/", today, "_Figure3A.svg"), plot = fig3a, height = 9, width = 12, units = "cm")
...
ggsave(file = paste0("Main figures/", today, "_Figure6A.svg"), plot = fig6a, width = 12, height = 9, units = "cm")

##########################################################

# Figure 7G
fig7g_data <- subset(dat.num, type == "ART" & ratio == "1-4-0")
fig7g_data$T_cell_facet <- factor(fig7g_data$T_cell, labels = c("bold(CD4^{'+'})", "bold(CD8^{'+'})"))
med_coord$T_cell_facet <- "bold(CD4^{'+'})"

fig7g <- ggplot(fig7g_data, aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.15) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  ...
  scale_x_continuous(limits = c(min(dat.num$UMAP_1), max(dat.num$UMAP_1))) +
  geom_density_2d(alpha = 0.3, color = "grey40", h = 3) + # Contour plots
  geom_label_repel(data = subset(med_coord, T_cell_facet == "bold(CD4^{'+'})"), aes(label = pheno_cluster), show.legend = F, size = 1.25, position = position_jitter()) # Add labels on map
fig7g

ggsave(file = paste0("Main figures/", today, "_Figure7G.svg"), plot = fig7g, height = 7, width = 6, units = "cm")

rm(fig3a_data, fig6a_data, fig7g_data) # Clean space
...
for (i in 9:17){
  ...
  normalized_values[,i] <- sapply(dat.num2[,i], function(x){
    (x-min)/(max-min)
  })
}
...
for (i in 9:17){
  ...
  umap_cytokines_CD4_norm[[tmp]] <- ggplot(normalized_values[normalized_values$T_cell == "CD4",], aes_string("UMAP_1", "UMAP_2", col = tmp)) +
    ...
    scale_color_gradientn(colours = rev(custom_pal), limits = c(min(normalized_values[,9:17]), max(normalized_values[,9:17])))
  # scale_color_gradientn(colours = rev(heat.colors(10)), limits = c(min(dat.num[,9:13]), max(dat.num[,9:13])))
}
fig_cytokines_CD4_norm <- do.call("ggarrange", c(umap_cytokines_CD4_norm, ncol = 3, nrow = 3, common.legend = T, legend = "right"))
annotate_figure(fig_cytokines_CD4_norm, bottom = "UMAP_1", left = "UMAP_2")
ggsave(file = paste0(today, "_FigureS3A_CD4.svg"), plot = annotate_figure(fig_cytokines_CD4_norm, bottom = "UMAP_1", left = "UMAP_2"), width = 20, height = 5, units = "cm")
...
fig_cytokines_CD8_norm <- do.call("ggarrange", c(umap_cytokines_CD8_norm, ncol = 3, nrow = 3, common.legend = T, legend = "right"))
annotate_figure(fig_cytokines_CD8_norm, bottom = "UMAP_1", left = "UMAP_2")
ggsave(file = paste0(today, "_FigureS3A_CD8.svg"), plot = annotate_figure(fig_cytokines_CD8_norm, bottom = "UMAP_1", left = "UMAP_2"), width = 20, height = 5, units = "cm")

##########################################################

# Organize data
sub2 <- normalized_values[,9:17] # Subset only phenotype channels
...
ggsave(file = paste0("Main figures/", today, "_Figure3B.svg"), plot = ph_normalized, width = 6.30, height = 6.30)
...
# Save as CSV
CSV_filename <- paste0(PrimaryDirectory, "/", today, "_unsupervised_ICS.csv")
...
write.table(d[,3:ncol(d)], CSV_filename, sep = ",", append = T)
# write.table(d_CD4[,3:ncol(d)], CSV_filename, sep = ",", append = T)
# write.table(d_CD8[,3:ncol(d)], CSV_filename, sep = ",", append = T)
