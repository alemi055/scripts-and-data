...
pheno_cluster <- pheno_cluster400
...
# Plot UMAP (total cells)
umap_CD4 <- ggplot(dat.num[dat.num$T_cell == "CD4",], aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.5) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  theme_bw() +
  labs(colour = "Cluster", title = expression(Total~CD4^{"+"}~T~cells)) +
  theme(plot.title = element_text(hjust = 0.5), # Center title
...
umap_CD8 <- ggplot(dat.num[dat.num$T_cell == "CD8",], aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.5) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  theme_bw() +
  labs(colour = "Cluster", title = expression(Total~CD8^{"+"}~T~cells)) +
  theme(plot.title = element_text(hjust = 0.5), # Center title
        legend.position = "none") +  # Remove legend
  scale_y_continuous(breaks = pretty_breaks(), limits = c(min(dat.num$UMAP_2), max(dat.num$UMAP_2))) + # To remove decimals
  scale_x_continuous(limits = c(min(dat.num$UMAP_1), max(dat.num$UMAP_1)))
...
umap_CD4_CD8 <- ggarrange(umap_CD4, umap_CD8, ncol = 2, nrow = 1, common.legend = T)
umap_CD4_CD8

##########################################################

# Plot UMAPs for each type only
umap_type <- ggplot(dat.num, aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.5) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  facet_grid(type ~ T_cell, switch = "y") +
  theme_bw() +
  labs(colour = "Cluster") +
...
# Plot UMAPs for each ratio only
umap_ratio <- ggplot(dat.num, aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.5) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  facet_grid(ratio ~ T_cell, switch = "y") +
  theme_bw() +
  labs(colour = "Cluster") +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "none", # Remove legend
...
# Change order of type and ratio
dat.num$type <- factor(dat.num$type, levels = c("UD", "CP", "ART"))
dat.num$ratio <- factor(dat.num$ratio, levels = c("1-0-4", "1-1-3", "1-4-0"))


# CD4
umap_ratio_type_CD4 <- ggplot(dat.num[which(dat.num$T_cell == "CD4"),], aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.5) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  facet_grid(type ~ ratio, switch = "y") +
  theme_bw() +
  labs(colour = "Cluster", title = expression(Total~CD4^{"+"}~T~cells)) +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "none", # Remove legend
...
# CD8
umap_ratio_type_CD8 <- ggplot(dat.num[which(dat.num$T_cell == "CD8"),], aes(UMAP_1, UMAP_2, col = pheno_cluster)) +
  geom_point(size = 0.5) +
  scale_color_discrete(drop = F) + # Keep same colour scheme was total cells graph
  facet_grid(type ~ ratio, switch = "y") +
  theme_bw() +
...
# Plot UMAPs for each cytokine
colnames(dat.num)[9:ncol(dat.num)] <- params

# CD4
umap_cytokines_CD4 <- list()
for (i in 9:13){
  tmp <- colnames(dat.num)[i]
  umap_cytokines_CD4[[tmp]] <- ggplot(dat.num[dat.num$T_cell == "CD4",], aes_string("UMAP_1", "UMAP_2", col = tmp)) +
    geom_point(size = 0.5) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(y = "", x = "", title = tmp, colour = "Value") +
    scale_color_gradientn(colours = rev(custom_pal), limits = c(min(dat.num[,9:13]), max(dat.num[,9:13])))
  # scale_color_gradientn(colours = rev(heat.colors(10)), limits = c(min(dat.num[,9:13]), max(dat.num[,9:13])))
}
fig_cytokines_CD4 <- do.call("ggarrange", c(umap_cytokines_CD4, ncol = 5, nrow = 1, common.legend = T, legend = "right"))
annotate_figure(fig_cytokines_CD4, bottom = "UMAP_1", left = "UMAP_2", top = text_grob(expression(Total~CD4^{"+"}~T~cells)))

# CD8
umap_cytokines_CD8 <- list()
for (i in 9:13){
  tmp <- colnames(dat.num)[i]
  umap_cytokines_CD8[[tmp]] <- ggplot(dat.num[dat.num$T_cell == "CD8",], aes_string("UMAP_1", "UMAP_2", col = tmp)) +
    geom_point(size = 0.5) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(y = "", x = "", title = tmp, colour = "Value") +
...
fig_cytokines_CD8 <- do.call("ggarrange", c(umap_cytokines_CD8, ncol = 5, nrow = 1, common.legend = T, legend = "right"))
annotate_figure(fig_cytokines_CD8, bottom = "UMAP_1", left = "UMAP_2", top = text_grob(expression(Total~CD8^{"+"}~T~cells)))

##########################################################

# Subset only channel MFIs and aggregate with mean across clusters
sub <- dat.num[,9:ncol(dat.num)] # Subset only phenotype channels
...
ph <- as.ggplot(pheatmap(d[,3:ncol(d)], scale = "column", border_col = 'white',
...
annotate_figure(fig_cytokines_CD4_norm, bottom = "UMAP_1", left = "UMAP_2", top = text_grob(expression(Total~CD4^{"+"}~T~cells)))
...
annotate_figure(fig_cytokines_CD8_norm, bottom = "UMAP_1", left = "UMAP_2", top = text_grob(expression(Total~CD8^{"+"}~T~cells)))
